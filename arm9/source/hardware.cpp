  ...
  while (!nds.gameTitle[0]) {
    ...
    while (!swap) {
      ...
      if (keys & KEY_A) {
        ...
        if (slot_1_type == AUXSPI_FLASH_CARD) continue;
...
bool swap_card_game(uint32 size) { return false; }
...
bool hwDetectSlot2DLDI() {
  ...
  // EZ Flash 4
  if (!strncasecmp(io_dldi_data->friendlyName, "EZ Flash 4", 10)) return true;
  ...
  // Ewin2
  if (!strncasecmp(io_dldi_data->friendlyName, "Ewin2", 5)) return true;

  // G6 Lite
  if (!strncasecmp(io_dldi_data->friendlyName, "G6 Lite DLDI", 12)) return true;
  ...
  if (!strncasecmp(io_dldi_data->friendlyName, "M3 Adapter", 10)) return true;
  ...
  // Neo2
  if (!strncasecmp(io_dldi_data->friendlyName, "Neo2", 4)) return true;

  // Supercard (CF/SD)
  if (!strncasecmp(io_dldi_data->friendlyName, "SuperCard (", 11)) return true;
...
// This function is called on boot; it detects the hardware configuration and
// selects the mode.
u32 hwDetect() {
  ...
  // First, look for a Slot 2 flash card. This must be done before any 3in1
  // Test, because
  ...
  //  we test for this. We look for a valid Slot 2 DLDI driver name, which
  //  pretty much ensures that we are writing to Slot 2.
  //
  // ... HOWEVER: We are also using a Slot 2 ini parameter, so WiFi mode can
  // also be
  ...
  if (gbaIsGame()) return 1;

  // Try to identify download play mode. This also introduces various
  // complications:
  // - The latest libnds versions include code for the SD-slot on the DSi. It
  // does not work
  //  from flash cards or download play, but it may still result in a positive
  //  DLDI driver initialisation. So we can't simply run "fatInitDefault" and
  //  test return values.
  // - Currently, download play mode aims to "insert game first, then run dlp",
  // i.e.
  //  it does not support hotswapping the game. So we select this mode if there
  //  is *no* flash card inserted in Slot 1.
  //
...
// --------------------------------------------------------
// uncomment this to enable slot 1 operations... but implement
// "dsiUnlockSlot1()" before! the following two functions are *untested* and may
// not work at all!
...
void hwBackupDSi() {
  ...
  int size_blocks =
      1 << max(0, (int8(size) -
                   18));  // ... in units of 0x40000 bytes - that's 256 kB
...
void hwRestoreDSi() {
  ...
  for (unsigned int i = 0; i < num_blocks; i++) {
    if (i % (num_blocks >> 6) == 0) displayProgressBar(i + 1, num_blocks);
...
void hwBackup3in1() {
  ...
  // This is necessary, since some DLDI drivers cease working after swapping a
  // card. Fix for dead batteries: don't write "reboot" values to SRAM, but to
  // NOR.
  displayMessage2F(STR_HW_3IN1_PREPARE_REBOOT);
  //
  sNDSHeader nds;
  cardReadHeader((u8 *)&nds);  // on a Cyclops Evolution, this call *will* mess
                               // up your DLDI driver!
  dsCardData data2;
  ...
  // We need to write the reboot flags to NOR, and we can only write in
  // 32kB-blocks, at aligned offsets. Therefore we prepare a bigger block and
  // copy lots of data now.
...
void hwRestore3in1() {
  ...
  int size_blocks =
      1 << max(
          0, int8(size) - 18);  // ... in units of 0x40000 bytes - that's 256 kB
...
bool hwRestoreFTPPartial(u32 ofs, u32 size, u32 type, netbuf *ndata) {
  ...
  for (int i = 0; i < (1 << (size - shift)); i++) {
    ...
    auxspi_write_data(ofs + (i << shift), ((u8 *)data) + (i << shift), LEN,
                      type, slot_1_type);
  }
...
////ENG_TEXT_END
/*//CHS_TEXT_START
    displayStateF(STR_STR, "…†üý!");//é…ä¿¡å®Œæˆ
*///CHS_TEXT_END
  }
}
...
void hwRestoreGBA() {
  ...
  if ((type == 1) || (type == 2)) {
    // This is not to be translated, it will be removed at some point.
    displayMessageF(STR_STR,
                    "I can't write this save type\nyet. Please use Rudolphs "
                    "tool\ninstead.");
    return;
  }
